...
class DeliveryBoyCrud(APIView):
    def post(self,request):
        ...
        if serializer.is_valid():
            ...
            if locserializer.is_valid():
                location = Point(locserializer.validated_data.get('longitude')
                ...
                return Response({'msg':'account created sucessfully'},status=status.HTTP_200_OK)
    ...
    def put(self,request):
        ...
        try:
            ...
            return Response({'msg':'delivery person account not fount'},status=status.HTTP_400_BAD_REQUEST)
        
        serializer = DeliveryPostSerializer(delivery_person,data=request.data,partial=True)
        if serializer.is_valid():
            serializer.save()
